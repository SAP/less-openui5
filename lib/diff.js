...
// `/*! ... */` important comments (postcss keeps the leading "!" as the first
// character of the comment text)
function isImportantComment(node) {
	return node && node.type === "comment" && /^\s*!/.test(node.text);
}

Diffing.prototype.pushToStack = function(oNode) {
	// A leading-comment count asymmetry between base and compare can shift a
	// marker pair so that one half is pushed here while the other is cloned into
	// the diff — orphaning it in the appended output. Never carry important
	// comments into the stack; the base file already emits the balanced pair.
	if (isImportantComment(oNode)) {
		return;
	}
...
Diffing.prototype.diffRules = function(oBaseRules, oCompareRules) {
	...
	for (iBaseNode = 0, iCompareNode = 0; iBaseNode < oBaseRules.length; iBaseNode++, iCompareNode++) {
		...
		if (oCompareNode && oBaseNode.type === "comment" && oCompareNode.type === "comment") {
			// Never emit important comments (banners / REDUCE markers) into the
			// diff; only genuine plain-comment changes belong there.
			if (!isImportantComment(oBaseNode) && !isImportantComment(oCompareNode) &&
					oBaseNode.text !== oCompareNode.text) {
