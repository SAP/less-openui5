 ...
 * @param {boolean} options.cssVars whether or not to enable css variables output
 * @param {string} options.lessInput less string input
 * @param {string} options.lessInputPath less file input
 * @returns {{css: string, cssRtl: string, variables: {}, imports: [], cssVars: {css: string, vars: string}}}
 */
Builder.prototype.build = function(options) {
	...
	// TODO: remove
	options.compiler.compress = false;
	...
	function compile(config) {
		...
		// inject the library name as prefix (e.g. "sap.m" as "_sapM")
		if (options.library && typeof options.library.name === "string") {
			config.prefix = "_" + options.library.name.replace(/\./g, "_");
		}
		...
		return new Promise(function(resolve, reject) {
			...
			if (options.cssVars) {
				return new Promise(function(resolve, reject) {
					...
					result.cssVars = {};
					result.cssVars.css = tree.toCSS(assign({}, options.compiler, {
						plugins: [oCSSVariablesCollector]
					}));
					result.cssVars.lessVariables = oCSSVariablesCollector.toLessVariables();
					if (oRTL) {
						const oCSSVariablesCollectorRTL = new CSSVariablesCollectorPlugin(config);
						result.cssVars.cssRtl = tree.toCSS(assign({}, options.compiler, {
					...
					return new Promise(function(resolve, reject) {
						parser.parse(result.cssVars.lessVariables, function(err, tree) {
							if (err) {
								reject(err);
							} else {
								result.cssVars.cssVariables = tree.toCSS(assign({}, options.compiler));
								resolve(result);
							}
						});
					});
		...
		return new Promise(function(resolve, reject) {
			if (typeof options.library === "object" && typeof options.library.name === "string") {
				...
				if (options.cssVars) {
					...
					result.cssVars.cssVariables += parameterStyleRule;
				}
			}
			resolve(result);
		});
	...
	function compileWithScope(scopeOptions) {
		...
		return Promise.all([
			...
			if (options.cssVars) {
				// results.embeddedCompare.cssVars.lessVariables += applyScope(results.embeddedCompare.cssVars.lessVariables, results.embedded.cssVars.lessVariables);
				results.embeddedCompare.cssVars.cssVariables += applyScope(results.embeddedCompare.cssVars.cssVariables, results.embedded.cssVars.cssVariables);
				results.embeddedCompare.cssVars.css += applyScope(results.embeddedCompare.cssVars.css, results.embedded.cssVars.css);
				if (options.rtl) {
					results.embeddedCompare.cssVars.cssRtl += applyScope(results.embeddedCompare.cssVars.cssRtl, results.embedded.cssVars.cssRtl, true);
				}
			}
