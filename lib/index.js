...
Builder.prototype.build = function(options) {
	...
	function fileHandler(file, currentFileInfo, handleDataAndCallCallback, callback) {
		...
		var pathname = path.posix.normalize(currentFileInfo.currentDirectory + file);
		...
		return that.fileUtils.readFile(dotThemingInputPath, options.rootPaths).then(function(result) {
			...
			if (dotTheming && dotTheming.mCssScopes) {
				...
				if (cssScope) {
					...
					return Promise.all([
						that.fileUtils.readFile(embeddedCompareFilePath, options.rootPaths).then(compile),
						that.fileUtils.readFile(embeddedFilePath, options.rootPaths).then(compile)
