...
Builder.prototype.build = function(options) {
	...
	function compile(config) {
		...
		return new Promise(function(resolve, reject) {
			...
			// TODO check this condition if customFs is the correct check
			if (options.rtl && that.customFs && options.library && options.library.name) {
				imgFilePaths = [];
				const imgExtensions = [".jpg", ".jpeg", ".png", ".gif"];
				const libraryRootDirname = "/resources/" + options.library.name.replace(/\./g, "/") + "/themes";

				relativeThemeDirname = path.posix.relative(libraryRootDirname, path.posix.dirname(filename));
				return that.fileUtils.findFilesByExtension(libraryRootDirname, imgExtensions, options.rootPaths).then(function(results) {
					imgFilePaths = results.map(function(imgFilePath) {
						return path.posix.relative(libraryRootDirname, imgFilePath);
					}).filter(function(imgFilePath) {
						return imgFilePath.includes("img-RTL/");
					});
					// imgFilePaths
			...
			// TODO check parsed less tree to get image paths
			...
			if (options.rtl) {
				...
				// get all images and pass to rtl plugin
				oRTL = new RTLPlugin(imgFilePaths, relativeThemeDirname);
			}
