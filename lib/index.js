...
function stringify(root, options) {
	if (options && options.compress) {
		// Minify by stripping only the *whitespace* from the cosmetic raws. `before`
		// is not purely cosmetic: postcss stores a leading property prefix such as the
		// IE underscore hack (`_border-left-color` → prop `border-left-color`, before `_`)
		// there, so blanking it outright would silently drop that prefix. Removing just
		// the whitespace keeps any meaningful non-whitespace content intact.
		root.walk(function(n) {
			n.raws.before = n.raws.before ? n.raws.before.replace(/\s+/g, "") : "";
			n.raws.after = n.raws.after ? n.raws.after.replace(/\s+/g, "") : "";
			if (n.type === "decl") {
				n.raws.between = ":";
	...
	function compileWithScope(scopeOptions) {
		...
		return Promise.all([
			...
			function applyScope(embeddedCompareCss, embeddedCss, bRtl) {
				...
				const oBase = postcss.parse(embeddedCompareCss);
				const oEmbedded = postcss.parse(embeddedCss);
				...
				if (oDiff.stack) {
					oCssScopeRoot = scope.scopeCssRoot(oDiff.stack.nodes, sScopeName);

					if (oCssScopeRoot) {
						oScope.prepend(oCssScopeRoot);
					}
				}
				...
				let sAppend = stringify(oScope, {
					compress: options.compiler && options.compiler.compress === true
				});

				if (scopeOptions.baseFilePath !== options.lessInputPath && oDiff.stack && oDiff.stack.nodes.length > 0) {
					sAppend += "\n" + stringify(oDiff.stack, {
						compress: options.compiler && options.compiler.compress === true
					});
				}
