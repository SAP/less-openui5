 ...
 * @param {boolean} options.compiler.cssVars whether or not to enable css variables output
...
Builder.prototype.build = function(options) {
	...
	function compile(config) {
		...
		return new Promise(function(resolve, reject) {
			...
			if (options.compiler.cssVars) {
				...
				result.cssVars.source = oCSSVariablesCollector.toLessVariables();
				if (oRTL) {
				...
				return new Promise(function(resolve, reject) {
					...
					parser.parse(result.cssVars.source, function(err, tree) {
						if (err) {
							reject(err);
						} else {
							result.cssVars.vars = tree.toCSS(assign({}, options.compiler));
							resolve(result);
						}
					});
				});
		...
		return new Promise(function(resolve, reject) {
			if (typeof options.library === "object" && typeof options.library.name === "string") {
				...
				if (options.compiler.cssVars) {
					...
					result.cssVars.vars += parameterStyleRule;
				}
			}
			resolve(result);
		});
	...
	function compileWithScope(scopeOptions) {
		...
		return Promise.all([
			...
			if (options.compiler.cssVars) {
				// results.embeddedCompare.cssVars.source += applyScope(results.embeddedCompare.cssVars.source, results.embedded.cssVars.source);
				results.embeddedCompare.cssVars.vars += applyScope(results.embeddedCompare.cssVars.vars, results.embedded.cssVars.vars);
