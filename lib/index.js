...
Builder.prototype.build = function(options) {
	...
	async function compile(config) {
		...
		const cssCompilePlugins = [oImportCollector, oVariableCollector, oUrlCollector];
		let oCSSVariablesCollector;
		if (options.inlineCssVariables) {
			oCSSVariablesCollector = new CSSVariablesCollectorPlugin(config);
			cssCompilePlugins.push(oCSSVariablesCollector);
		}
		// render to css
		result.css = tree.toCSS(Object.assign({}, options.compiler, {
			plugins: cssCompilePlugins
		}));

		if (options.inlineCssVariables) {
			result.cssVariablesOnly = oCSSVariablesCollector.getCssVariablesDeclaration();
		}
...
`;
			result.css += "\n" + additionalVariables + result.cssVariablesOnly;
			if (options.rtl) {
				// TODO: Do we need a cssVariablesOnlyRtl variant?
				result.cssRtl += "\n" + additionalVariables + result.cssVariablesOnly;
			}
		}
