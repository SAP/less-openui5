...
tree.functions = {
    ...
    "data-uri": function(mimetypeNode, filePathNode) {
        ...
        if (arguments.length < 2) {
            var mime, charsetLookup;
            try {
                mime = require('mime');
                charsetLookup = function(mimeType) {
                    // Assume text types are utf8
                    return (/^text\/|^application\/(javascript|json)/).test(mimeType) ? 'UTF-8' : undefined;
                };
            } catch (ex) {
                mime = tree._mime;
                charsetLookup = function (type) {
            ...
            var charset = charsetLookup(mimetype);
