  ...
  getVariables: function(aImports) {
    ...
    for (var name in this.mVariables) {
      if (this.mVariables.hasOwnProperty(name)) {
        ...
        var dirname = path.dirname(oVar.filename);
        var baseFileName = path.basename(oVar.rootFilename); // usually library.source.less
        var libraryBaseFile = path.normalize(path.join(dirname, baseFileName));
