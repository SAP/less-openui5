...
CSSVariablesCollectorPlugin.prototype = {
	...
	toLessVariables: function() {
		let lessVariables = "";
		Object.keys(this.vars).forEach(function(value, index) {
	...
	visitOperation: function(node, visitArgs) {
		// console.log("visitOperation", node, visitArgs);
		if (!this._isInMixinOrParen() && this._isVarInRule()) {
			return new less.tree.Call("calc", [new less.tree.Expression([node.operands[0], new less.tree.Anonymous(node.op), node.operands[1]])]);
		}
		return node;
	},

	visitCall: function(node, visitArgs) {
		// if variables are used inside rules, generate a new dynamic variable for it!
		if (!this._isInMixinOrParen() && this._isVarInRule() && typeof less.tree.functions[node.name] === "function" && ["rgba"].indexOf(node.name) === -1) {
			const newName = this.config.prefix + "_function_" + node.name + Object.keys(this.vars).length;
			this.vars[newName] = {
				css: this._getCSS(node)
			};
	...
	visitNegative: function(node, visitArgs) {
	...
	visitMixinDefinitionOut: function(node) {
		// remove mixin context
		this.mixinStack.pop();
		return node;
	},

	visitParen: function(node, visitArgs) {
		// store the parenthesis context
		this.parenStack.push(node);
		return node;
	},

	visitParenOut: function(node) {
	...
	visitRuleset: function(node, visitArgs) {
		node.rules.forEach(function(value) {
			if (!this._isInMixinOrParen() && value instanceof less.tree.Rule && typeof value.name === "string" && value.name.startsWith("@")) {
				// add the variable declaration to the list of vars
