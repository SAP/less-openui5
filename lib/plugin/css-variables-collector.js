...
const CSSVariablesCollectorPlugin = module.exports = function(config, options) {
	this.config = config;
	// Only replace nodes when a library skeleton should be created.
	// This allows to also use the plugin in the default compilation
	// for the library.css in order to collect the content for the
	// inline CSS Variables feature.
	this.isReplacing = options && options.createLibrarySkeleton === true;
	...
	this.mVars = {}; // variable assignments for getCssVariablesDeclaration
};
...
CSSVariablesCollectorPlugin.prototype = {
	...
	visitRule(node, visitArgs) {
		...
		if (!this._isInMixinOrParen() && isVarDeclaration) {
			...
			const filename = node.currentFileInfo.filename;
			const isVarInLib = this._isVarInLibrary({filename});
			const varEntry = {
				css: this._getCSS(node.value),
				export: isVarInLib
			};
			this.vars[varName] = varEntry;

			if (varEntry.export) {
				// Create additional _asResolvedUrl variable for runtime resolution of relative urls
				const urlMatch = /url[\s]*\('?"?([^'")]*)'?"?\)/.exec(varEntry.css);
				if (urlMatch) {
					...
					if (resolvedUrl) {
						const resolvedUrlVariableName = `${varName}__asResolvedUrl`;
