...
const converterFunctions = {
	...
	url: function(ruleNode) {
		// only replace if file is there in path
		const imgPaths = this.imgPaths;
		const relativeThemeDirname = this.relativeThemeDirname;
		ruleNode.value.value.forEach(function(valueObject) {
			if (valueObject.type === "Url") {
				replaceUrl(valueObject, imgPaths, relativeThemeDirname);
			} else if (valueObject.type === "Expression") {
				valueObject.value.forEach(function(childValueObject) {
					replaceUrl(childValueObject, imgPaths, relativeThemeDirname);
				});
			}
		});
	},
...
/**
 *
 * @param {string} imgPath, e.g. ../base/img/Busy.gif
 * @param {string[]} imgPaths, e.g. ["base/img-RTL/Busy.gif"]
 * @param {string} relativeThemeDirname, e.g. "mine"
 * @returns {boolean} whether the mirrored image is part of the given imgPaths
 */
function isMirroredImagePresent(imgPath, imgPaths, relativeThemeDirname) {
	if (!imgPaths) {
		return true;
	}
	const imgPathRTL = imgPath.replace(urlPattern, urlReplacement);
	// ../base/img-RTL/Busy.gif

	const imgPathRTLNormalized = path.posix.join(relativeThemeDirname, imgPathRTL);
	// base/img-RTL/Busy.gif

	return imgPaths.includes(imgPathRTLNormalized);
}

function replaceUrl(node, imgPaths, relativeThemeDirname) {
	// note: paths might be relative
	// background-image: url(../base/img/Busy.gif);
	// background-image: url(img/Busy.gif);
	if (node.type === "Url" && isMirroredImagePresent(node.value.value, imgPaths, relativeThemeDirname)) {
 ...
 *
 * @param {string[]} imgPaths, e.g. ["base/img-RTL/Busy.gif"]
 * @param {string} relativeThemeDirname, e.g. "base"
 * @constructor
 */
const LessRtlPlugin = module.exports = function(imgPaths, relativeThemeDirname) {
	/* eslint-disable new-cap */
	this.oVisitor = new less.tree.visitor(this);
	this.imgPaths = imgPaths;
	this.relativeThemeDirname = relativeThemeDirname;
